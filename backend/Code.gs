...
function doGet(e){
  ...
  try{
    ...
    if(action==='provaTentativa') return json_(provaTentativa_(p.provaId||PROVA_EM_01,p.nome,p.email));
    if(action==='ping') return json_({ok:true,sistema:'1º TANE',versao:'1.1',componentes:COMPONENTES,agora:new Date().toISOString()});
...
function listar_(turma,componente){
  ...
  const materiais=rows_(ABAS.MATERIAIS)
    ...
    .sort((a,b)=>Number(a.ORDEM||999)-Number(b.ORDEM||999))
    .map(m=>({id:m.ID_MATERIAL,titulo:m.TITULO,descricao:m.DESCRICAO,url:m.ARQUIVO_URL,tipo:m.TIPO,status:m.STATUS,publicadoEm:dateTimeOut_(m.PUBLICADO_EM)}));
...
function atividade_(id,componente){
  if(!id)return null;
  const c=comp_(componente);
  const a=rows_(ABAS.ATIVIDADES).find(x=>eq_(x.ID_ATIVIDADE,id)&&eq_(x.TURMA,TURMA)&&eq_(String(x.COMPONENTE||'').toUpperCase(),c));
...
function mapAtividadePublica_(a){
  ...
  return {
    ...
    tipoEnvio:a.TIPO_ENVIO||'SEM_ENVIO',extensoes:a.EXTENSOES||'',maxArquivos:Number(a.MAX_ARQUIVOS||0),
    liberacao:dateTimeOut_(a.LIBERACAO),prazo:dateTimeOut_(a.PRAZO),materialUrl:a.MATERIAL_APOIO_URL||'',
    status:status,ordem:Number(a.ORDEM||999),situacao:situacao,disponivel:situacao==='ABERTA'
  };
}
...
function enviarAtividade_(d){
  ...
  const isTexto=['FORMULARIO','TEXTO'].includes(String(atividade.tipoEnvio||'').toUpperCase());
  if(!isTexto&&atividade.tipoEnvio!=='SEM_ENVIO'){
  ...
  if(isTexto&&!String(d.respostaTexto||'').trim())throw new Error('Digite a resposta da atividade');
  validaArquivos_(arquivos,atividade.extensoes);

  const cfg=config_();
  const pastaId=cfg['PASTA_ENTREGAS_'+c+'_ID']||cfg.PASTA_ENTREGAS_ID;
...
function provaSheets_(){
  ...
  const pd=sheetData_(provas);if(!pd.rows.some(r=>eq_(r.PROVA_ID,PROVA_EM_01)))provas.appendRow([PROVA_EM_01,'EM','1ª Prova — Economia e Mercado','NAO',new Date()]);
  return {provas,tent};
}
function provaConfig_(provaId){
  const ps=provaSheets_(),r=sheetData_(ps.provas).rows.find(x=>eq_(x.PROVA_ID,provaId));
  return {liberada:String((r&&r.CORRECAO_LIBERADA)||'NAO').toUpperCase()==='SIM'};
}
function tentativaRow_(provaId,email){
  const sh=provaSheets_().tent,d=sheetData_(sh);const e=String(email||'').trim().toLowerCase();
  const r=d.rows.find(x=>eq_(x.PROVA_ID,provaId)&&String(x.EMAIL||'').trim().toLowerCase()===e);
  return {sh,data:d,row:r};
}
function mencao_(acertos){const n=Number(acertos||0);if(n>=18)return'MB';if(n>=16)return'B';if(n>=9)return'R';return'I'}
function provaTentativa_(provaId,nome,email){
  if(!String(email||'').trim())return {ok:true,encontrada:false};
  const t=tentativaRow_(provaId,email),r=t.row;if(!r)return {ok:true,encontrada:false};
  const cfg=provaConfig_(provaId),finished=String(r.STATUS||'').toUpperCase()==='FINALIZADA';
  const out={ok:true,encontrada:true,idTentativa:r.ID_TENTATIVA,provaId:r.PROVA_ID,nome:r.ALUNO,email:r.EMAIL,status:r.STATUS,finished,acertos:finished?Number(r.ACERTOS||0):null,mencao:finished?String(r.MENCAO||''):null,correcaoLiberada:cfg.liberada};
  if(!finished){out.answers=parseJson_(r.RESPOSTAS_JSON,{});out.order=parseJson_(r.ORDEM_JSON,[]);out.altOrder=parseJson_(r.ALTERNATIVAS_JSON,{});out.startedAt=dateTimeOut_(r.INICIO_EM)}
  if(finished&&cfg.liberada)out.correcao=montarCorrecao_(r);
  return out;
}
function salvarProva_(d){
  const provaId=String(d.provaId||PROVA_EM_01),nome=String(d.nome||'').trim(),email=String(d.email||'').trim().toLowerCase();
  if(!nome||!email)throw new Error('Nome e e-mail são obrigatórios');
  const t=tentativaRow_(provaId,email),sh=t.sh,now=new Date();
  if(t.row&&String(t.row.STATUS||'').toUpperCase()==='FINALIZADA')return {ok:true,finished:true,acertos:Number(t.row.ACERTOS||0),mencao:String(t.row.MENCAO||'')};
  const obj={ID_TENTATIVA:t.row?t.row.ID_TENTATIVA:'PRV-'+Utilities.getUuid().slice(0,12).toUpperCase(),PROVA_ID:provaId,ALUNO:nome,EMAIL:email,STATUS:'EM_ANDAMENTO',INICIO_EM:t.row&&t.row.INICIO_EM?t.row.INICIO_EM:now,ATUALIZADO_EM:now,FINALIZADO_EM:'',RESPOSTAS_JSON:JSON.stringify(d.answers||{}),ORDEM_JSON:JSON.stringify(d.order||[]),ALTERNATIVAS_JSON:JSON.stringify(d.altOrder||{}),SNAPSHOT_JSON:t.row?t.row.SNAPSHOT_JSON||'':'',ACERTOS:'',MENCAO:'',EMAIL_CORRECAO_EM:t.row?t.row.EMAIL_CORRECAO_EM||'':''};
  const headers=sheetData_(sh).headers;
  if(t.row)sh.getRange(t.row.__row,1,1,headers.length).setValues([headers.map(h=>obj[h]!==undefined?obj[h]:'')]);else sh.appendRow(headers.map(h=>obj[h]!==undefined?obj[h]:''));
  return {ok:true,idTentativa:obj.ID_TENTATIVA,finished:false};
}
function finalizarProva_(d){
  const provaId=String(d.provaId||PROVA_EM_01),nome=String(d.nome||'').trim(),email=String(d.email||'').trim().toLowerCase(),answers=d.answers||{};
  if(!nome||!email)throw new Error('Nome e e-mail são obrigatórios');
  if(Object.keys(answers).length<20)throw new Error('A prova ainda possui questões não respondidas');
  const t=tentativaRow_(provaId,email),sh=t.sh,now=new Date();
  if(t.row&&String(t.row.STATUS||'').toUpperCase()==='FINALIZADA')return {ok:true,finished:true,acertos:Number(t.row.ACERTOS||0),mencao:String(t.row.MENCAO||'')};
  let acertos=0;Object.keys(GABARITO_EM_01).forEach(q=>{if(String(answers[q]||'')===GABARITO_EM_01[q])acertos++});const mencao=mencao_(acertos);
  const obj={ID_TENTATIVA:t.row?t.row.ID_TENTATIVA:'PRV-'+Utilities.getUuid().slice(0,12).toUpperCase(),PROVA_ID:provaId,ALUNO:nome,EMAIL:email,STATUS:'FINALIZADA',INICIO_EM:t.row&&t.row.INICIO_EM?t.row.INICIO_EM:(d.startedAt||now),ATUALIZADO_EM:now,FINALIZADO_EM:now,RESPOSTAS_JSON:JSON.stringify(answers),ORDEM_JSON:JSON.stringify(d.order||[]),ALTERNATIVAS_JSON:JSON.stringify(d.altOrder||{}),SNAPSHOT_JSON:JSON.stringify(d.snapshot||[]),ACERTOS:acertos,MENCAO:mencao,EMAIL_CORRECAO_EM:t.row?t.row.EMAIL_CORRECAO_EM||'':''};
  const headers=sheetData_(sh).headers;if(t.row)sh.getRange(t.row.__row,1,1,headers.length).setValues([headers.map(h=>obj[h]!==undefined?obj[h]:'')]);else sh.appendRow(headers.map(h=>obj[h]!==undefined?obj[h]:''));
  return {ok:true,finished:true,acertos,mencao,correcaoLiberada:provaConfig_(provaId).liberada};
}
function montarCorrecao_(r){
  const answers=parseJson_(r.RESPOSTAS_JSON,{}),snapshot=parseJson_(r.SNAPSHOT_JSON,[]);if(!Array.isArray(snapshot))return[];
  return snapshot.map((q,i)=>{const marcada=String(answers[q.id]||''),correta=String(GABARITO_EM_01[q.id]||''),opts=Array.isArray(q.options)?q.options:[],find=id=>opts.find(o=>String(o.id)===id)||null;return {numero:i+1,id:q.id,text:q.text||'',marcada,correta,marcadaTexto:(find(marcada)||{}).text||'',corretaTexto:(find(correta)||{}).text||'',acertou:marcada===correta}});
}

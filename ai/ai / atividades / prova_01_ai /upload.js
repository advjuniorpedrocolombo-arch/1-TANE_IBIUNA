...
(function(){
  ...
  panel.innerHTML=`
    <div class="upload-head"><small>Entrega da avaliação</small><h2>Enviar arquivo PDF</h2></div>
    <div class="upload-body">
      <div class="upload-note"><strong>📄 Envio obrigatório em PDF.</strong><br>Selecione o trabalho final exportado do Word ou editor similar. Limite máximo: 15 MB.</div>
  ...
  function saveAttempt(a){localStorage.setItem(PROVA_AI_STORAGE_KEY,JSON.stringify(a))}
  ...
  fileInput.addEventListener('change',()=>{
    ...
    if(!file){meta.textContent='Nenhum arquivo selecionado.';return}
    const isPdf=file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf');
    if(!isPdf){meta.textContent='Arquivo inválido. Selecione somente PDF.';setStatus('error','Somente arquivos PDF são aceitos.');return}
    if(file.size>PROVA_AI_MAX_BYTES){meta.textContent='Arquivo acima de 15 MB.';setStatus('error','O PDF excede o limite máximo de 15 MB.');return}
  ...
  sendBtn.addEventListener('click',async()=>{
    ...
    const file=fileInput.files&&fileInput.files[0];
    if(!attempt){setStatus('error','Tentativa não localizada. Retome ou reinicie a prova antes de enviar.');return}
    if(!file){setStatus('error','Selecione o arquivo PDF antes de enviar.');return}
    if(attempt.submitted){setStatus('ok',`Esta prova já foi enviada. Protocolo: ${attempt.protocol||'registrado'}.`);return}
    ...
    try{
      ...
      const payload={
        ...
        tema:'Formatação acadêmica ABNT',
        mimeType:'application/pdf',
        pdfBase64,
        observacoes:'Envio realizado pela interface da 1ª Prova de Aplicativos Informatizados.'
      };

      const response=await fetch(PROVA_AI_ENDPOINT,{
        method:'POST',
        redirect:'follow',
      ...
      let result={};
      try{result=JSON.parse(text)}catch(e){throw new Error('O servidor respondeu em formato inesperado. Tente novamente.')}
      if(!result.ok)throw new Error(result.erro||'Não foi possível concluir o envio.');

      attempt.submitted=true;
      attempt.protocol=result.protocolo||'';
      attempt.submittedAt=Date.now();
      saveAttempt(attempt);
      setStatus('ok',`✓ PDF enviado com sucesso. Protocolo: ${result.protocolo}. Aguarde a devolutiva no e-mail informado.`);
      sendBtn.textContent='PDF ENVIADO COM SUCESSO';
    }catch(err){
  ...
  if(existing&&existing.submitted){
    fileInput.disabled=true;
    sendBtn.disabled=true;
    sendBtn.textContent='PDF JÁ ENVIADO';
    setStatus('ok',`✓ Esta prova já foi enviada. Protocolo: ${existing.protocol||'registrado'}.`);
  }
})();

...
function doPost(e) {
  ...
  try {
    lock.waitLock(30000);
    if(!e||!e.postData||!e.postData.contents) throw new Error('Requisição sem conteúdo.');
    ...
    if(pdfBytes.length>CONFIG.MAX_PDF_BYTES) throw new Error('O PDF excede o limite de 15 MB.');

    const arquivo=DriveApp.getFolderById(CONFIG.PASTA_RESPOSTAS_PDF_ID)
    ...
    const horarioInicio=inicio&&!isNaN(inicio.getTime())?Utilities.formatDate(inicio,CONFIG.TIMEZONE,'dd/MM/yyyy HH:mm:ss'):'';
    const horarioEnvio=Utilities.formatDate(agora,CONFIG.TIMEZONE,'dd/MM/yyyy HH:mm:ss');

    sheet.appendRow([
      protocolo,horarioInicio,nomes,String(data.email||'').trim(),CONFIG.TURMA,
      data.versao||'',data.tema||data.titulo||'',horarioEnvio,tempoMin,
      CONFIG.TEMPO_REFERENCIA_MINUTOS,excedente,nomeArquivo,arquivo.getUrl(),
      'PENDENTE','','','','','','','','','',montarObservacoes_(data,aluno1,aluno2),''
    ]);

    return json_({ok:true,protocolo,arquivo:nomeArquivo,url:arquivo.getUrl(),horarioEnvio});
  } catch(err) {
...
function validar_(data,aluno1,aluno2){
  ...
  if(!aluno1||aluno1.length<3) throw new Error('Informe o nome completo do primeiro aluno.');
  ...
  if(String(data.mimeType||'application/pdf').toLowerCase()!=='application/pdf') throw new Error('Somente arquivos PDF são aceitos.');
}
...
function validarEmail_(email){return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);}
function removerPrefixoBase64_(valor){const s=String(valor||'');const idx=s.indexOf('base64,');return idx>=0?s.substring(idx+7):s;}
function limparNome_(nome){return String(nome||'Aluno').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9 _-]/g,'').replace(/\s+/g,' ').trim().substring(0,80)||'Aluno';}
function gerarProtocolo_(){const agora=new Date();const data=Utilities.formatDate(agora,CONFIG.TIMEZONE,'yyyyMMdd-HHmmss');const aleatorio=Math.floor(1000+Math.random()*9000);return `ATD2-AI-${data}-${aleatorio}`;}
